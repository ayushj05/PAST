...
void PastryNode::start_node(string ip, string port){
    string node_id = hash4(ip, port);

    info = {node_id, ip, port};

    cout << "\n    NodeID: " << node_id << " | IP: " << ip  << " | Port: " << port << "\n" << endl;
    
    threads.push_back(std::thread( [this] { this->node_server(); } ));
    threads.push_back(std::thread( [this] { this->check_peers(); } ));
}

void PastryNode::stop_node(){
    for(auto& th : threads){
        if(th.joinable())
            th.join();
    }
...
int PastryNode::connectTo(string ip, string port){
    int client_fd;
...
void PastryNode::handleRequest(int server_fd, int client_fd){
...
bool PastryNode::route(const char* buffer, string nodeID, string ip /*= "_"*/, string port /*= "_"*/){
    if(ip == "_"){
...
// buffer = "LSet <sender's nodeID> <sender's ip> <sender's port> <sender's leaf set>"
void PastryNode::set_LSet(char* buffer){
    char* saveptr;
...
void PastryNode::insert_LSet(string nodeID, string ip, string port){
...
// buffer = "RT <row_index> <row_index^th row for RT>"
void PastryNode::set_RTable(char* buffer){
...
string PastryNode::getRow(int row_index){
    shared_lock<shared_mutex> lock(RTable_mtx);
...
void PastryNode::store_key_value(string key, string value){
    lock_guard<shared_mutex> lock(storage_mtx);
    storage[key] = value;
}

string PastryNode::get_value(string key){
    shared_lock<shared_mutex> lock(storage_mtx);
    return storage[key];
}

void PastryNode::delete_key_value(string key){
    lock_guard<shared_mutex> lock(storage_mtx);
    storage.erase(key);
}

string PastryNode::getID(){
    return info.nodeID;
}
